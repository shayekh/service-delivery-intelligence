...
class PdfBuilder {
  ...
  private drawFooter(): void {
    const text = `${this.projectName} | ${this.period} | Page ${this.pageNumber} | Generated by Service Delivery Intelligence`;
    safeDrawText(this.page, text, {
      x: MARGIN,
      y: MARGIN - 20,
      size: 8,
      font: this.regular,
  ...
  drawSectionHeader(number: string, title: string): void {
    ...
    this.y -= 6;
    safeDrawText(this.page, `S${number} — ${title}`, {
      x: MARGIN,
      y: this.y,
      size: 15,
      font: this.bold,
  ...
  drawCrossAnalysisList(
    ...
    items.forEach((entry) => {
      ...
      const tagSize = 7;
      const tagPaddingX = 5;
      const tagWidth = this.bold.widthOfTextAtSize(entry.relationship, tagSize) + tagPaddingX * 2;
      const tagHeight = 12;
      ...
      safeDrawText(this.page, entry.relationship, {
...
export async function generateReportPdf(projectId: string): Promise<string> {
  ...
  if (s1) {
    ...
    builder.spacer(20);
    const prose = [s1.delivery_focus, s1.highlights, s1.areas_requiring_attention, s1.next_quarter_preview]
      .filter(Boolean)
      .join(" ");
    builder.drawParagraph(prose);
  } else {
